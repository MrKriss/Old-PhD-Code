...
def plotEqqFqq32(streams, Q_t, alpha, p = 0, flag = 0):
    """
...
def genSimSignals32(randSeed, SNR):
...
if __name__ == '__main__':
    ...
    Sim_streams = genSimSignals32(0, -3.0)
    ...
    No_inp_marker = FRHH32(Sim_streams, rr, alpha, sci)  
    
    # Calculate deviations from orthogonality and subspace
    e_qq, f_qq, g_qq = plotEqqFqq32(Sim_streams, Q_t, alpha, 1)
    
    fRHH32_data = {'alpha' : alpha,
                   'sci' : sci,
                   'Q_t' : Q_t,
                   'S_t' : S_t,
                   'rr' : rr,
