...
def FRAHST_V2(streams, energyThresh, alpha, sci = 0, init_Q = 0):
    """
    ...
    if init_Q != 0:
        ...
        q_0, r_0 = qr(rand(NumStreams,rr[0]))   # generate random orthonormal matrix N x r 
        Q_t = [q_0]         
        
    # S_0
    S_t = [eye(rr[0]) * 0.00001]
    ...
    for t in range(1, streams.shape[0] + 1):
        ...
        if E_dash_t[-1] < energyThresh[0] * E_t[-1] and rr[-1] < NumStreams: # 13 m 
            ...
            z_dag_orthog =  z_t - dot( dot(Q_t[t], Q_t[t].T), z_t) 
        
            # Update Q[t] by adding another column
            Q_t[t] = hstack((Q_t[t], z_dag_orthog/norm(z_dag_orthog)))
                                             
            TR = zeros((S_t[t].shape[0], 1)) # top left
            top_row = hstack((S_t[t], TR))                
            BL = zeros((1 ,S_t[t].shape[1])) # bottom left
            ##### NOTE CHANGED NORM TO SUM #####
            BR = array([[sum(z_dag_orthog) ** 2 ]])  # bottom right 
            bottom_row = hstack((BL, BR))
            # Update S[t] by adding another row and another column
            S_t[t] = vstack((top_row, bottom_row))
...
if __name__ == '__main__' : 
    ...
    = FRAHST_V2(streams, energyThresh, alpha, sci)
