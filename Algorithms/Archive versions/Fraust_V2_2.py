...
vstack, hstack
from numpy.random import rand, randn , seed, nan
from numpy.linalg import qr, eig, norm
from matplotlib.pyplot import plot, figure, title
from QRsolve import QRsolveA
from SimpleCosSig import genSimSignals

def FRAHST_V2(streams, energyThresh, alpha, sci = 0, fix_init_Q = 0):
    """
    ...
    hidden_var = zeros((timeSteps, NumStreams)) * nan # Array of hidden Variables     
    rr = [1]
    
    # cummulative sum variables
    last_z_minus_z_dash_norm = 0.0
    last_z_norm = 0.0
    
    # Initialisations 
    # Q_0
    if fix_init_Q != 0:  # fix inital Q as identity 
        q_0 = eye(NumStreams,rr[0]);
        Q_t = [q_0]
    else: # generate random orthonormal matrix N x r 
        q_0, r_0 = qr(rand(NumStreams,rr[0]))   
        Q_t = [q_0]         
        
    # S_0
    S_t = [eye(rr[0]) * 0.0001] # Avoids Singularity
    
    # v-1
    v_minus_1 = zeros((rr[0],1))    
    ...
    for t in range(1, timeSteps + 1):
        ...
        if Z > 0 :
            ...
            if sci != 0: 
                u_vec = dot(S_t[-1] , v_minus_1) 
                extra_term = 2 * alpha * sci * dot(u_vec, v_minus_1.T)
            else:
                extra_term = 0            
            
            X = alpha * S_t[-1] + dot(h_t , h_t.T) - extra_term
            ...
            ee = delta * z_t - dot(Q_t[-1] , w)
            
            new_Q_t = Q_t[-1] - 2 * dot(ee , v.T) 
            Q_t.append(new_Q_t) 
        ...
        new_z_dash = dot(Q_t[-1] , h_t)
        z_dash = vstack((z_dash, new_z_dash.T))
    
        # Record RSRE
        top = last_z_minus_z_dash_norm + (norm(z_t - new_z_dash) ** 2 )
        bot = last_z_norm + (norm(z_t) ** 2)
                                    
        RSRE = vstack((RSRE, top / bot)) 
    
        # Update cumulative variables
        last_z_minus_z_dash_norm = top     
        last_z_norm = bot         
        ...
        # tx = float(t)            
        #new_Et = (((tx - 1.0) / tx) * alpha * E_t[-1]) + ((1.0/tx) * (norm(z_t) ** 2))              
        new_Et = alpha * E_t[-1] + sum(z_t ** 2) 
        
        E_t.append(new_Et)                                    # 13 k
            
        #new_E_dash_t = (((tx-1.0)/tx)*alpha*E_dash_t[-1]) + ((1.0/tx) * (norm(h_t) ** 2)) 
        new_E_dash_t = alpha * E_dash_t[-1] + sum(new_z_dash ** 2) 

        E_dash_t.append(new_E_dash_t)                          # 13 l
    
        # Adjust Q_t ans S_t for change in rr 
        if E_dash_t[-1] < energyThresh[0] * E_t[-1] and rr[-1] < NumStreams: # 13 m 
    
            z_dag_orthog =  z_t - dot( dot(Q_t[-1], Q_t[-1].T), z_t) 
        
            # Update Q[t] by adding another column
            Q_t[-1] = hstack((Q_t[-1], z_dag_orthog/norm(z_dag_orthog)))
                                             
            TR = zeros((S_t[-1].shape[0], 1)) # top right
            top_row = hstack((S_t[-1], TR))                
            BL = zeros((1 ,S_t[-1].shape[1])) # bottom left
            ##### NOTE CHANGED NORM TO SUM #####
            BR = array([[norm(z_dag_orthog) ** 2 ]])  # bottom right 
            bottom_row = hstack((BL, BR))
            # Update S[t] by adding another row and another column
            S_t[-1] = vstack((top_row, bottom_row))
            
            # Record new rr, increment
            rr.append(rr[-1] + 1)
            
            if sci != 0:  # Add zero to bottom of v
                v_minus_1 = vstack((v_minus_1, array([0])))
                 
        elif E_dash_t[-1] > energyThresh[1] * E_t[-1] and rr[-1] > 1 :
            
            # delete the last column of Q_t
            Q_t[-1] = Q_t[-1][:, :-1]   
            
            # delete last row and colum of S_t 
            S_t[-1] = S_t[-1][:-1, :-1] 
    
            # record new rr, decrement
            rr.append(rr[-1] - 1) 
            
            if sci != 0: # delete last element/row 
                v_minus_1 = v_minus_1[:-1,:] # update for next time step
        
        else: # Keep rr the same as before, record  
            rr.append(rr[-1])         
...
if __name__ == '__main__' : 
    ...
    streams = genSimSignals(timesteps = 10, N = 10)    
    ...
    = FRAHST_V2(streams, energyThresh, alpha, sci, init_Q = 1)
