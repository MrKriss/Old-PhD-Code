...
Experiment to generate data for two types of experiment:

    peak: a number c of anomalous peaks (short transiant anomalies) are added to random streams and at random positions
          The streams are otherwise N in phase noisy sinosoidal streams. 
          
    shift: a number n of the N streams shift from one pattern of input to another (longer persistent change), 
          and then switch back after t time steps . Shift is to 
                a: just noise (complete signal drop), 
                b: a change in phase. 
          The start point for each stream is at random position, but same for n streams. 


          TODO: Code up everything, bellow is copy of Anom peakOR shift code to hack. 
